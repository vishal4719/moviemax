...
export const Header = () => {
  ...
  return (
    <header>
      <nav className="bg-white border-gray-200 dark:bg-gray-900 dark-b-1">
        <div className="max-w-screen-xl flex flex-wrap items-center justify-between mx-auto p-4">
          <Link to="/" className="flex items-center space-x-3 rtl:space-x-reverse">
            <img src={Logo} className="h-10 bg-black dark:bg-gray-900 rounded" alt="MovieMax Logo" />
            <span className="self-center text-2xl font-semibold whitespace-nowrap dark:text-white">MovieMax</span>
          </Link>
          {/* this div contains mobile nav mode */}
          ...
          <div id="nav-links" className={`${hidden ? "" : "hidden"} items-center justify-between w-full md:flex md:w-auto md:order-1`}>
            ...
            <ul className="flex flex-col p-4 md:p-0 mt-4 font-medium border border-gray-100 rounded-lg bg-gray-50 md:space-x-8 rtl:space-x-reverse md:flex-row md:mt-0 md:border-0 md:bg-white dark:bg-gray-800 md:dark:bg-gray-900 dark:border-gray-700">
              <li>
                <NavLink to="/" className={({ isActive }) => (isActive ? activeClass : inActiveClass)} onClick={handleNavLinkClick}>Home</NavLink>
              </li>
              <li>
                <NavLink to="/movies/popular" className={({ isActive }) => (isActive ? activeClass : inActiveClass)} onClick={handleNavLinkClick}>Popular</NavLink>
              </li>
              <li>
                <NavLink to="/movies/top" className={({ isActive }) => (isActive ? activeClass : inActiveClass)} onClick={handleNavLinkClick}>Top Rated</NavLink>
              </li>
              <li>
                <NavLink to="/movies/upcoming" className={({ isActive }) => (isActive ? activeClass : inActiveClass)} onClick={handleNavLinkClick}>Upcoming</NavLink>
              </li>
            </ul>
          </div>
        </div>
